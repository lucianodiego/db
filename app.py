...
# --- ROTTA DI SETUP (Resa più veloce e sicura) ---
...
def setup_online_db():
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            # Crea la tabella metadata se non esiste
            cur.execute("""
            ...
            # Inserisce il valore di default per last_update solo se non esiste già
            cur.execute("""
                INSERT INTO metadata (key, value) VALUES ('last_update', 'Nessun caricamento eseguito')
...
def index():
    ...
    if is_search_active:
        ...
        for label, text in active_fields.items():
            if label != 'data_nascita' and len(text) < 3:
                invalid_fields.append(label)
        
        # Validazione per l'anno di nascita: se presente, deve essere di 4 numeri.
        anno_testo = active_fields.get('data_nascita')
        if anno_testo and not (len(anno_testo) == 4 and anno_testo.isdigit()):
            invalid_fields.append('data_nascita')

        if invalid_fields:
            field_labels = {'cognome': 'Cognome', 'nome': 'Nome', 'data_nascita': 'Anno di Nascita (deve essere composto da 4 numeri)', 'nome_padre': 'Nome del Padre', 'nome_madre': 'Nome della Madre'}
...
if __name__ == "__main__":
    app.run(debug=True)
>>>>>>> d90b58218496f46fbcbefafee9cbf141d9d8c529
